...
if [[ -d "/sys/class/power_supply/BAT0" ]]; then
    ...
    get_icon() {
        ...
        if [[ "$current" =~ ^[0-9]+$ ]]; then
            icon_step=$(( (current + 5) / 10 * 10 ))
            ((icon_step > 100)) && icon_step=100
            ((icon_step < 0)) && icon_step=0
            icon="$iDIR/brightness-${icon_step}.png"
        else
            icon="$iDIR/brightness-50.png"
        fi
        [[ ! -f "$icon" ]] && icon="$HOME/.config/hypr/icons/done.png"
    }
else
    device="Desktop"
