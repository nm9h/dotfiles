...
if [[ "$next_mode" == "light" ]]; then
    ...
    nvim_cs="$HOME/.config/nvim/lua/shell-ninja/plugins/colorscheme.lua"
    [[ -f "$nvim_cs" ]] && sed -i 's/mocha/latte/g' "$nvim_cs"

    for css in "$HOME/.config/waybar/style/fancy-top.css" "$HOME/.config/waybar/style/full-top.css"; do
        [[ -f "$css" ]] && sed -i 's/rgba(0, 0, 0, 0.5)/rgba(255, 255, 255, 0.5)/g' "$css"
    done

    gsettings set org.gnome.desktop.interface gtk-theme "Religh" 2>/dev/null || true
    if command -v crudini &>/dev/null && [[ -f "$HOME/.config/Kvantum/kvantum.kvconfig" ]]; then
    ...
    nvim_cs="$HOME/.config/nvim/lua/shell-ninja/plugins/colorscheme.lua"
    [[ -f "$nvim_cs" ]] && sed -i 's/latte/mocha/g' "$nvim_cs"

    for css in "$HOME/.config/waybar/style/fancy-top.css" "$HOME/.config/waybar/style/full-top.css"; do
        [[ -f "$css" ]] && sed -i 's/rgba(255, 255, 255, 0.5)/rgba(0, 0, 0, 0.5)/g' "$css"
    done

    gsettings set org.gnome.desktop.interface gtk-theme "TokyoNight" 2>/dev/null || true
    if command -v crudini &>/dev/null && [[ -f "$HOME/.config/Kvantum/kvantum.kvconfig" ]]; then
