...
# Scale for monitor
mon_x_res=$(hyprctl -j monitors 2>/dev/null | jq -r '.[] | select(.focused==true) | .width' 2>/dev/null | head -n1)
mon_scale=$(hyprctl -j monitors 2>/dev/null | jq -r '.[] | select(.focused==true) | .scale' 2>/dev/null | head -n1)

[[ -z "$mon_x_res" || "$mon_x_res" == "null" || "$mon_x_res" == "0" ]] && mon_x_res=1920
[[ -z "$mon_scale" || "$mon_scale" == "null" || "$mon_scale" == "0" ]] && mon_scale=1

mon_x_res=$(awk -v x="$mon_x_res" -v s="$mon_scale" 'BEGIN { printf "%d", (x / s) }')
